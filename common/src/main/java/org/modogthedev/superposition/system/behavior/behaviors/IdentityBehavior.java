...
public class IdentityBehavior extends Behavior implements ScanBehavior {
    public IdentityBehavior(ResourceLocation selfReference) {
        super(selfReference);
    }
    ...
    public void scan(CompoundTag tag, AnalyserBlockEntity analyserBlockEntity, Level level, BlockPos pos, BlockState state) {
        ...
        tag.putString(getSelfReference().getPath(),state.getBlock().getName().getString());
    }
}
